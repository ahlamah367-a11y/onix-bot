...
أضف:
    # 🔄 استعادة الفعاليات بعد إعادة تشغيل البوت
...
لكن انتبه: إذا on_ready عندك ممكن يشتغل أكثر من مرة، الأفضل نمنع تشغيل الـtasks مرتين. استخدم متغير:
...
وحط داخل on_ready:
    ...
    if not EVENTS_RESTORED:
        ...
        asyncio.create_task(restore_events())
⭐ ومهم جدًا بالنسبة للـXP الموجود عندك
عندك حاليًا في on_message:
xp_data[guild_id][user_id_str]["xp"] += 1
لكن أنت لا تحفظ xp_data بعد التعديل، لذلك الـXP الحالي ممكن يضيع عند Restart.
بعد زيادة الـXP، أضف:
save_json(XP_FILE, xp_data)
فيصير:
xp_data[guild_id][user_id_str]["xp"] += 1
save_json(XP_FILE, xp_data)
ضيفلي دا ع دا الكود الاثلي ولاتمحي شي من الكود الاصلي ترا اهينك
import os
